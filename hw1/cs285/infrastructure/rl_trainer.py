...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        if not self.params['agent_params'].get('discrete', False):
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            if self.log_video or self.log_metrics:
                ...
                eval_avg, eval_std, train_avg, train_std, init = self.perform_logging(
                    itr, paths, eval_policy, train_video_paths, training_logs)
                self.evaluations.append(eval_avg)
                self.evaluations.append(eval_std)
                self.evaluations.append(train_avg)
                self.evaluations.append(train_std)
                self.evaluations.append(init)
    ...
    def collect_training_trajectories(
        ...
        if itr == 0 :
          pathfile = open(load_initial_expertdata, "rb")
          paths = pickle.load(pathfile)
          return paths, 0, None
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, training_logs):
        ...
        eval_AverageReturn = 0
        eval_StdReturn = 0
        train_AverageReturn = 0
        train_StdReturn = 0
        initialReturn = 0
        ...
        if self.log_metrics:
            ...
            if itr == 0:
                ...
                initialReturn = np.mean(train_returns)
            ...
            # Plot training loss
            training_loss_history = np.array([lg['Training Loss'] for lg in training_logs])
            fig = plt.figure()
            plt.plot(training_loss_history)
            plt.title('Training loss')
            # plt.show()
            self.logger.log_figure(fig, 'loss', itr, 'training')

            self.logger.flush()
        
        return eval_AverageReturn, eval_StdReturn, train_AverageReturn, train_StdReturn, initialReturn
